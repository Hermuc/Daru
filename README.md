...
已安装 Scoop 与 Git（Git 可用 `winget install Git.Git` 安装）。
...
软件清单见 `bucket/` 目录（每个 .json 即一个软件），也可用 `scoop search` 查询。
...
1. 参考已有 manifest：结构最全为 `QQ.json` / `WeChat.json`（junction 持久化 + checkver/autoupdate 全套）；AppData 数据较轻的 Flutter 系应用参考 `Kazumi.json` / `PiliPlus.json`
2. 编写 `bucket/<包名>.json`：文件名即软件名（如 `QQ.json`），小写、无空格
3. 本地验证（见「提交前验证」）
...
4. 注册表共享视图（`HKLM\SOFTWARE` 与 `WOW6432Node` 指向同一数据）：第二处删除加 `-ErrorAction SilentlyContinue`（仅涉注册表清理的 manifest 内嵌脚本）；内嵌脚本慎用 `$ErrorActionPreference='Stop'`，避免单点失败中断后续步骤（独立交互式脚本不受此限）
5. **无硬编码路径**：只用 `$dir` / `$version` / `$persist_dir` 等变量；`bin` / `shortcuts` 只写 current 内的相对路径（文件名），绝对路径在版本更新后会变死链
6. **GUI 启动包装器**（需先设环境变量再启动，如 HypoMux 的 HYPOMUX_DATA_DIR）：shortcut 指向 `.vbs`（wscript 窗口样式 0，零黑窗闪现；vbs 必须 ASCII——WScript 按 ANSI 读取）；`bin` 用 `.cmd`（scoop shim 对 `.vbs` 无专门分支，会生成无效 shim）；内嵌生成 vbs 用 `Chr(34)` 拼引号，避免双重转义
7. **面向用户的脚本须双击可运行且可移植**（仅维护者终端使用的脚本不受此限）——详见「🖱️ 脚本双击运行与可移植性」

### 💾 数据持久化（本仓库核心能力）

1. 设置类数据：用 `persist` 字段声明到 `$persist_dir`（Scoop 自动处理）
2. AppData 登录态（微信 / QQ / FlClash 等）：installer 内联 `Ensure-LinkedData` 三态函数——链接不存在 → 创建 junction 指向 persist 目录；已是 junction → 幂等跳过；真实目录有数据 → `robocopy /E /COPY:DAT /DCOPY:DAT` 迁移（勿用 `/COPYALL`，ACL 权限会失败）→ 原目录改名 `.bak-<时间戳>` 作回滚点 → 建 junction
...
4. 解包统一用 scoop 内置 `Expand-7zipArchive`（无需 `depends`）；nanazip 是零 7zip 特例，用 .NET `ZipFile` 原生解包、URL 不带 `#/dl.7z`
...
自动更新管线在更新后逐应用执行 `scoop cleanup <应用> -k`（删旧版本目录与过期缓存，保留 current 与 persist；实现与部署方法见 `bucket/scripts/auto-update/`）。**必须逐应用调用而非 `scoop cleanup *`**——后者遇被进程占用的文件（如常驻后台工具）会报错中断，后续应用都不被清理。新增软件须满足：
...
4. **部署前置条件须在脚本输出和 README 中说明**：如脚本须位于 Scoop 根目录（含 `apps\` 与 `shims\`），启动时校验并给出清晰报错，不得默默失败
5. **编码要求**：`.cmd` / `.bat` 纯 ASCII（cmd.exe 按 OEM 代码页读取，中文易乱码），其余同铁律 1、6
6. **README / 脚本头注释须说明运行方式**：哪些可双击（推荐入口）、哪些需终端运行、部署步骤与前置条件
...
3. **excavator 自动更新重写 manifest 时可能移除 BOM 并损坏中文注释**（Kazumi 2.2.8 实证：`首次/迁移` 变 `�?`）——已自动化：`.github/workflows/excavator.yml` 的 `verify-integrity` job 在每次自动更新后校验全部 manifest 的 BOM（`EF BB BF`）与中文完整性（U+FFFD 检测），异常即 workflow 失败告警；发现损坏用 `git checkout <更新前提交> -- <文件>` 从历史恢复重提（仅缺 BOM 时直接补回文件头）
